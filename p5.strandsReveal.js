...
//
// API (all optional, call when you want):
...
function strandsReveal(p5, fn) {
  ...
  p5.Shader.prototype.modify = function (cb, scope, options) {
    ...
    if (captured) {
      ...
      } catch (_) { /* shader may be frozen; ignore */ }
  ...
  p5.strandsReveal = {
    get last() { return lastCapture; },
    get history() { return captures.slice(); },
    log(record) {
      const r = record || lastCapture;
      if (!r) {
        console.log('[strandsReveal] no transpiled callback captured yet');
        return;
      }
      console.group('[strandsReveal] transpiled strands callback');
      console.log('--- original source ---');
      console.log(r.original);
      console.log('--- transpiled JS (what strands actually executes) ---');
      console.log(r.formatted);
      console.groupEnd();
    },
    clear() { captures.length = 0; lastCapture = null; },
  };
  ...
  fn.getStrandsTranspile = function (shader) {
    if (shader && shader._strandsTranspiledJS) return shader._strandsTranspiledJS;
    return lastCapture;
  };
}
